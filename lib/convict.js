...
function normalizeSchema(name, node, props, fullName, provider, env, argv, sensitive) {
  ...
  // associate this property with a provider
  if (externalProvider.get) {
    if (!provider[o.providerPath]) {
      provider[o.providerPath] = [];
    }
    provider[o.providerPath].push(fullName);
  }
...
function importProvider(o) {
  if(externalProvider.get){
    Object.keys(o._provider).forEach(function(externalString) {
      if (externalProvider.get(externalString) !== undefined) {
        let ks = o._provider[externalString];
        ks.forEach(function(k) {
          o.set(k, String(externalProvider.get(externalString)));
        });
      }
    });
  }
...
let convict = function convict(def) {
  ...
  Object.keys(rv._def).forEach(function(k) {
    normalizeSchema(k, rv._def[k], rv._schema.properties, k, rv._provider, rv._env, rv._argv,
      rv._sensitive);
  });
...
/**
   * Adds an exernal provider
*/
convict.configureProvider = function(p) {
  externalProvider.get = p;
