...
function overlay(from, to, schema, instance) {
  Object.keys(from).forEach(function(k) {
    // leaf
    if (Array.isArray(from[k]) || !isObj(from[k]) || !schema || schema.format === 'object') {
      to[k] = coerce(k, from[k], schema, instance);
    } else {
      if (!isObj(to[k])) to[k] = {};
      overlay(from[k], to[k], schema.properties[k], instance);
    }
  });
}
...
var convict = function convict(def) {
  ...
  var rv = {
    ...
    load: function(conf) {
      overlay(conf, this._instance, this._schema, this);
    ...
    loadFile: function(paths) {
      ...
      paths.forEach(function(path) {
        overlay(cjson.load(path), self._instance, self._schema, self);
      });
      // environment and arguments always overrides config files
      importEnvironment(rv);
      importArguments(rv);
