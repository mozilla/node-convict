...
// format can be a type, as seen below
// or an array of enumerated values
...
var types = {
  int: function(name, x) {
    check(x, name + ': must be an integer').isInt();
  },
  nat: function(name, x) {
    check(x, name + ': must be a positive integer').isInt().min(0);
  },
  port: function(name, x) {
    check(x, name + ': Ports must be within range 1 - 65535').isInt().min(1).max(65535);
  },
  url: function(name, x) {
    check(x, name + ': must be a URL').isUrl();
  },
  email: function(name, x) {
    check(x, name + ': must be an email address').isEmail();
  },
  ipaddress: function(name, x) {
    check(x, name + ': must be an IP address').isIP();
  },
  duration: function(name, x) {
    var err = name + ': must be a positive integer or human readable string (e.g. "2 weeks")';
    if (typeof x === 'number') {
      check(x, err).isInt().min(0);
    } else {
      check(x, err).regex(/^d+ .+$/);
    }
  },
  date: function(name, x) {
    if (!moment().isValid(x)) {
      throw new Error(name + ': must be a date object, timestamp, or string');
    }
  }
};
...
function contains (options, name, x) {
  check(x, name + ': Instance is not one of the possible values: ' + JSON.stringify(options)).isIn(options);
}
  ...
  var rv = {
    ...
    set: function(k, v) {
      ...
      if (typeof v === 'string') {
        ...
        case 'date': v = moment(v); break;
        case 'duration':
          var split = v.split(' ');
          v = moment.duration(parseInt(split[0], 10), split[1]);
          break;
        }
      }
      var ar = k.split('.');
