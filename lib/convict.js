...
var types = {
  int: function(x) {
    check(x, 'must be an integer, got ' + JSON.stringify(x)).isInt();
  },
  nat: function(x) {
    check(x, 'must be a positive integer, got ' + JSON.stringify(x)).isInt().min(0);
  },
  port: function(x) {
    check(x, 'Ports must be within range 1 - 65535, got ' + JSON.stringify(x)).isInt().min(1).max(65535);
  },
  url: function(x) {
    check(x, 'must be a URL, got ' + JSON.stringify(x)).isUrl();
  },
  email: function(x) {
    check(x, 'must be an email address, got ' + JSON.stringify(x)).isEmail();
  },
  ipaddress: function(x) {
    check(x, 'must be an IP address, got ' + JSON.stringify(x)).isIP();
  },
  duration: function(x) {
    check(x, 'must be a positive integer, got ' + JSON.stringify(x)).isInt().min(0);
  },
...
function sanityCheck (instance, schema, errorCb) {
  Object.keys(schema.properties).map(function(name) {
    var p = schema.properties[name];
    if (p.properties) {
      var kids = instance[name] || {};
      sanityCheck(kids, p, errorCb);
    } else if (p.sanityCheck) {
      p.sanityCheck(instance[name], errorCb);
    }
  });
}
...
function contains (options, x) {
  check(x, 'Instance is ' + JSON.stringify(x) + ', not one of the possible values: ' + JSON.stringify(options)).isIn(options);
}
  ...
  if (typeof o === 'object') {
    ...
    if (BUILT_INS.indexOf(format) >= 0) {
      ...
      newFormat = function(x) {
        check(x.constructor, 'must be an instance of ' + format).equals(format);
      };
      ...
      newFormat = function (x) { contains(format, x); };
      o.type = format;
    ...
    if (!newFormat && !o.format) {
      // default format is the typeof the default value
      var type = typeof o.default;
      newFormat = function(x) { check(typeof x, ' should be of type ' + type).equals(type); };
    }

    // attach the property's fullName to the error
    o.format = function (x) {
      try {
        newFormat(x);
      } catch(e) {
...
function getSanityChecker(schema, path) {
  var o = traverseSchema(schema, path);
  return o ? o.sanityCheck : null;
}
...
module.exports = function convict(def) {
  var rv = {
    ...
    sanityCheck: function(path, errorCb) {
      if (typeof path === 'function') {
        sanityCheck(this._instance, this._schema, path);
      } else {
        var checker = getSanityChecker(this._schema, path);
        if (checker) {
          checker(this.get(path), errorCb || function(err) { if (err) console.error(err); });
        } else {
          throw new Error("cannot find sanity checker for configuration param '" + path + "'");
        }
      }
    }
  };
